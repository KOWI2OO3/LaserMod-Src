...
public class Dropdown extends Button {
	...
	public Dropdown(int posX, int posY, int width, int height, String... list) {
		super(posX, posY, width, height, new StringTextComponent(""), (button) -> {
	...
	public void setList(String... list) {
		...
		for(int i = 0; i < list.length; i++) {
			buttons.add(new Button(posX, (i+1) * height + posY, width, height, new StringTextComponent(list[i]), (button) ->  {
	...
	public void setList(List<String> list) {
		...
		for(int i = 0; i < list.size(); i++) {
			buttons.add(new Button(posX, (i+1) * height + posY, width, height, new StringTextComponent(list.get(i)), (button) ->  {
				currentIndex = this.buttons.indexOf(button);
			}));
		}
	}
	...
	public void addElement(String element) {
		buttons.add(new Button(posX, (buttons.size()) * height + posY, width, height, new StringTextComponent(element), (button) ->  {
			currentIndex = this.buttons.indexOf(button);
		}));
	}
