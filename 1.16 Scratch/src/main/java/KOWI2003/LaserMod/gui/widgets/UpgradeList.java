...
public class UpgradeList extends Button {
	...
	public UpgradeList(int x, int y, int width, int height,
			ITextComponent title, IPressable pressable, String... strings) {
		super(x, y, width, height, title, pressable);
		for (int i = 0; i < strings.length; i++) {
			buttons.add(new Button(x, y + i * height, width, height, new StringTextComponent(strings[i]), pressable));
		}
	}
	
	public UpgradeList(int x, int y, int width, int height,
			ITextComponent title, IPressable pressable, ResourceLocation Texture, String... strings) {
	...
	public void replaceList(String... names) {
		...
		for (int i = 0; i < names.length; i++) {
			buttons.add(new Button(x, y + i * height, width, height, new StringTextComponent(names[i]), onPress));
		}
		if(buttons.size() > 3)
			cropList();
	}
	...
	public void AddElement(String name) {
		buttons.add(new Button(x, y + buttons.size() * height, width, height, new StringTextComponent(name), onPress));
		if(buttons.size() > 3)
			cropList();
	}
	...
	public void AddElements(String... names) {
		for (int i = 0; i < names.length; i++) {
			buttons.add(new Button(x, y + (i + buttons.size()) * height, width, height, new StringTextComponent(names[i]), onPress));
		}
		if(buttons.size() > 3)
			cropList();
	}
	
	public void renderButton(Button button, MatrixStack matrix, int x, int y, float partialticks, ResourceLocation TEXTURE) {
		Minecraft minecraft = Minecraft.getInstance();
	    FontRenderer fontrenderer = minecraft.font;
	    minecraft.getTextureManager().bind(TEXTURE);
	    RenderSystem.color4f(1.0F, 1.0F, 1.0F, this.alpha);
	    int i = this.getYImage(button.isHovered());
...
//	    button.renderBg(matrix, minecraft, x, y);
	    int j = getFGColor();
	    drawCenteredString(matrix, fontrenderer, button.getMessage(), button.x + button.getWidth() / 2, button.y + (button.getHeight() - 8) / 2, j | MathHelper.ceil(this.alpha * 255.0F) << 24);
	    if (button.isHovered()) {
