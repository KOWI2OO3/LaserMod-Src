		...
		if(getInputs(te).length >= 2) {
			if(getInputs(te)[0] != null && getInputs(te)[1] != null && getOutput(te) != null) {
				...
				if(getInputs(te)[0] instanceof ItemStack)
					...
					con1 = te.handler.getStackInSlot(0).getItem().is(tag) && te.handler.getStackInSlot(0).getCount() > 0;
				}else
				...
				else if(getInputs(te)[1] instanceof ITag<?>) {
					ITag<Item> tag = (ITag<Item>)getInputs(te)[1];
					con2 = te.handler.getStackInSlot(1).getItem().is(tag) && te.handler.getStackInSlot(1).getCount() > 0;
				}else
