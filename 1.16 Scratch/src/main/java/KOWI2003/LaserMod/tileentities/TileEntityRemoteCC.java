...
public class TileEntityRemoteCC extends TileEntityLaserController {
	...
	public TileEntityRemoteCC() {
		super(ModTileTypes.LASER_CONTROLLER_CC);
	...
	public int getConnectedType(BlockPos pos) {
		...
		if(block == ModBlocks.Laser)
			return 1;
		if(block == ModBlocks.LaserProjector)
			return 2;
		if(block == ModBlocks.AdvancedLaser)
			return 3;
		return -1;
	}
	
	public TileEntityAdvancedLaser getAdvancedLaserControlTileEntity() {
		TileEntity tileentity = getControlTileEntity();
		if(tileentity instanceof TileEntityAdvancedLaser)
			return (TileEntityAdvancedLaser) tileentity;
		return null;
	}
	
	public void setAdvancedLaserRotation(Vector2f rotation) {
		TileEntityAdvancedLaser te = getAdvancedLaserControlTileEntity();
		if(te == null)
			return;
		...
		switch( index )
        {
			case 0:
			{
				...
				if(tileentity instanceof TileEntityLaser)
					((TileEntityLaser)tileentity).setActive((boolean)args[0]);
				else if(tileentity instanceof TileEntityLaserProjector)
					((TileEntityLaserProjector)tileentity).setActive((boolean)args[0]);
				break;
			}
			...
			case 4:
			{
				...
				TileEntity tileentity = getControlTileEntity();
				if(tileentity instanceof TileEntityLaser)
					((TileEntityLaser)tileentity).setColor(r, g, b);
				break;
			}
			...
			case 5:
			{
				...
				TileEntity tileentity = getControlTileEntity();
				if(tileentity instanceof TileEntityLaser) {
					if(((TileEntityLaser)tileentity).properties.hasUpgarde("mode")) {
						((TileEntityLaser)tileentity).mode = MODE.values()[mode];
						((TileEntityLaser)tileentity).sync();
					}
				}else if(tileentity instanceof TileEntityLaserProjector) {
					((TileEntityLaserProjector)tileentity).mode = PROJECTOR_MODES.values()[mode];
			...
			case 8:
			{
				...
				if(tileentity instanceof TileEntityLaserProjector) {
					((TileEntityLaserProjector)tileentity).setText(text);
				}
				break;
			}
        	...
        	case 9:
        	{
        		...
        		TileEntity tileentity = getControlTileEntity();
				if(tileentity instanceof TileEntityLaserProjector) {
					((TileEntityLaserProjector)tileentity).properties.setProperty(PROJECTOR_PROPERTY.values()[propIndex], value);
					((TileEntityLaserProjector)tileentity).sync();
				}
				break;
        	}
        	...
        	case 12:
        	{
				...
				if(tileentity instanceof TileEntityLaserProjector) {
					...
					{ ((TileEntityLaserProjector)tileentity).properties.setProperty(prop, prop.getDefaultValue()); }
					((TileEntityLaserProjector)tileentity).sync();
				}
				break;
        	}
        	...
        	case 14:
        	{
        		...
        		if(te == null)
        			break;
        		te.setRotationEular(new Vector2f(x, y));
        		break;
        	}
