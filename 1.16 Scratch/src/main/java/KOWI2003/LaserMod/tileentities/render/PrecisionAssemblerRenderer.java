...
public class PrecisionAssemblerRenderer extends TileEntityRenderer<TileEntityPrecisionAssembler> {

	public PrecisionAssemblerRenderer(TileEntityRendererDispatcher rendererDispatcherIn) {
		super(rendererDispatcherIn);
	}

	@Override
	public void render(TileEntityPrecisionAssembler te, float partialTicks, MatrixStack matrix,
		...
		if(!te.handler.getStackInSlot(slot).isEmpty()) {
			...
			}else if(te.handler.getStackInSlot(slot).getItem() instanceof ItemLaserToolBase || te.handler.getStackInSlot(slot).getItem() == ModItems.LaserToolShell) {
				scaler = 0.7f;
				matrix.translate(-0.570f, 0.058f, 0.115f);
				matrix.mulPose(Vector3f.XP.rotationDegrees(90));
				matrix.mulPose(Vector3f.YP.rotationDegrees(36.1f));
			}else if(te.handler.getStackInSlot(slot).getItem() == ModItems.LaserCrystal) {
				scaler = scaler/2f;
