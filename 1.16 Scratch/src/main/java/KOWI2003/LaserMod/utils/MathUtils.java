...
public class MathUtils {
	...
	public static Vector3f getPosWithRotation(ModelRenderer render) {
		MatrixStack matrix = new MatrixStack();
		matrix.translate((double)(render.x), (double)(render.y), (double)(render.z));
		if (render.zRot != 0.0F) {
			matrix.mulPose(Vector3f.ZP.rotation(render.zRot));
		}
		
		if (render.yRot != 0.0F) {
			matrix.mulPose(Vector3f.YP.rotation(render.yRot));
		}
		
		if (render.xRot != 0.0F) {
			matrix.mulPose(Vector3f.XP.rotation(render.xRot));
		}
		
		Matrix4f matrix4f = matrix.last().pose();
	    Matrix3f matrix3f = matrix.last().normal();
		
	    ModelBox cube = render.getRandomCube(new Random());
	...
	public static float getAngleNormalized(Vector2f vec1, Vector2f vec2) {
		vec1 = normalize(vec1);
		vec2 = normalize(vec2);
	...
	public static double getAngle(Vector3f vec1, Vector3f vec2, String interpereter) {
		Vector2f _vec1 = getVec2f(vec1, interpereter);
		Vector2f _vec2 = getVec2f(vec2, interpereter);
	...
	public static Vector2f getVec2f(Vector3f vec1, String interpereter) {
		...
		return new Vector2f(_vec1X, _vec1Y);
	}
	...
	public static Vector3f forwardToEuler(Vector3f forward, Vector3f up) {
		return new Vector3f(ToEuler(forwardToQuaternion(forward, up)));
	}
	...
	public static double getLenght(Vector2f vec) {
		return Math.sqrt(vec.x * vec.x + vec.y * vec.y);
	}
	
	public static double getLenghtSqr(Vector2f vec) {
	...
	public static double getInn(Vector2f vec1, Vector2f vec2) {
	...
	public static Vector2f mulVector(Vector2f vec, float scalar) {
